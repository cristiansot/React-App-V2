...
function WeeklyProgressChart({ currentWeek, originalApiData }) {
  ...
  // -------------useEffect For Calculating Chart Data------------//
  useEffect(() => {
  ...
  // -------------Function That Processes The API Data And Calculates Weekly Totals For Each Activity------------//
  const calculateOriginalChartDataInfo = (apiData, currentWeek) => {
    ...
    apiData.forEach(activity => {
      ...
      if (doesWeekStartMatchCurrentWeek) {
        ...
        weeklyTotals[weekRange] = weeklyTotals[weekRange] || Object.fromEntries(activityOptions.map(option => [option.activity, 0]));
